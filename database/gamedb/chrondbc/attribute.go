...
// Attribute encodes both the block index and bitmask.
// Layout: upper 32 bits = block index, lower 32 bits = bitmask
type Attribute uint64

// Block returns the attribute block index (0-8).
func (a Attribute) Block() int { return int(a >> 32) }

// Mask returns the bitmask for this attribute within its block.
func (a Attribute) Mask() uint32 { return uint32(a) }
...
func (sa *SpellAttributes) Clear(a Attribute) {
	sa[a.Block()] &^= a.Mask()
}

// SpellAttributesFromSpell extracts attributes from an Ent_Spell.
func SpellAttributesFromSpell(s *dbdefs.Ent_Spell) SpellAttributes {
	return SpellAttributes{
		uint32(s.Attributes),
		uint32(s.AttributesEx),
		uint32(s.AttributesExB),
		uint32(s.AttributesExC),
		uint32(s.AttributesExD),
		uint32(s.AttributesExE),
		uint32(s.AttributesExF),
		uint32(s.AttributesExG),
		uint32(s.AttributesExH),
	}
}
...
const (
	AttrEx2_CanTargetDead                Attribute = (1 << iota) | (2 << 32) //  0 can target dead unit or corpse
	AttrEx2_Unk1                                                             //  1
...
const (
	AttrEx3_OutOfCombatAttack          Attribute = (1 << iota) | (3 << 32) //  0 Spell landed counts as hostile action against enemy even if it doesn't trigger combat state, propagates PvP flags
	AttrEx3_Unk1                                                           //  1
	AttrEx3_Unk2                                                           //  2
...
const (
	AttrEx4_IgnoreResistances          Attribute = (1 << iota) | (4 << 32) //  0
